          ...
          {dias.map(dia => (
            <Link key={dia.numero} href={`/diario/dia-${String(dia.numero).padStart(2, '0')}`} style={{ display: 'block' }}>
              <article style={{ position: 'relative', aspectRatio: '4/3', background: 'var(--sand)', overflow: 'hidden', cursor: 'pointer' }}>
                <div style={{ position: 'absolute', inset: 0, background: `url(/images/dia-${String(dia.numero).padStart(2, '0')}/capa.jpg) center/cover no-repeat`, transition: 'transform 0.4s ease' }} className="card-img" />
